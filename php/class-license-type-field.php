...
/**
 * Class License_Type_Field
 *
 * The License Type Field for Gravity Forms
 *
...
class License_Type_Field extends \GF_Field {

	public $type = 'license_type';
	...
	public function run() {
		...
		\GF_Fields::register( new \mkdo\license_type_field_for_gravity_forms\License_Type_Field() );
	}
	...
	public function gform_editor_js_set_default_values() {
		?>
		case 'license_type' :
			field.label = '<?php _e( 'License Type', MKDO_LFGF_TEXT_DOMAIN ); ?>';
			field.inputs = [
				new Input( field.id + 0.1, "<?php _e( 'License Type ID', MKDO_LFGF_TEXT_DOMAIN ); ?>" ),
				new Input( field.id + 0.2, "<?php _e( 'License Type Text', MKDO_LFGF_TEXT_DOMAIN ); ?>" )
			];
		break;
		<?php
	}
	 ...
	 *
	 * Note that the class on the li ('license_type_setting' in this case) will
	...
	public function gform_field_standard_settings( $position ) {
		if ( 25 == $position  ) {
			...
			?>
			<li class="license_type_setting field_setting">
				<label for="license_type_admin_label" class="section_label">
					<?php _e( 'Choose License Type', MKDO_LFGF_TEXT_DOMAIN ); ?>
					<?php gform_tooltip( 'license_type' ) ?>
				</label>
				<select id="license_type" onchange="SetFieldProperty( 'license_type', this.value ); ToggleLicenseType( this.value ); ">
...
jQuery(document).bind( 'gform_load_field_settings', function( event, field, form ) {
	jQuery( '#license_type').val( field.license_type == undefined ? '' : field.license_type );
});
function ToggleLicenseType( type ) {
	var field = GetSelectedField(),
		isSubmitButton = type == 'submit',
		id_value = jQuery( '#license_type option:selected' ).val();
		text_value = jQuery( '#license_type option:selected' ).text();
	...
	public function gform_tooltips( $tooltips ) {
		$tooltips['license_type'] = __( 'Choose a License Type', TEXTDOMAIN );
		return $tooltips;
	}
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'License Type', MKDO_LFGF_TEXT_DOMAIN );
	}
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		 ...
		 */
		$type_id_label   = rgar( $type_id_input, 'customLabel' ) != '' ? $type_id_input['customLabel'] : gf_apply_filters( array( 'license_type_id', $form_id ), esc_html__( 'Licence Type ID', MKDO_LFGF_TEXT_DOMAIN ), $form_id );
		$type_text_label = rgar( $type_text_input, 'customLabel' ) != '' ? $type_text_input['customLabel'] : gf_apply_filters( array( 'license_type_text', $form_id ), esc_html__( 'Licence Type Text', MKDO_LFGF_TEXT_DOMAIN ), $form_id );
